        ...
        .with_event_loop(move |ctx, setter, _window| unsafe {
            while let Ok(value) = ctx.next_event() {
                if let Ok(message) = serde_json::from_value::<GuiMessage<GM>>(value.clone()) {
                    match message {
                        GuiMessage::Init => {
                            params.param_map().iter().for_each(|(id, param, _)| {
                                pm_channel_editor
                                    .0
                        ...
                        GuiMessage::ParamChange(param_change) => {
                            let param = if let Some(param) = params
                                .param_map()
                                .iter()
                                .find(|(id, _, _)| id == &param_change.id)
                            {
                                param.1
                            } else {
...
{}"#,
    ...
    pub fn with_message_handler(mut self, handler: impl Fn(GM) + 'static + Send + Sync) -> Self {
        todo!();
    }
...
where
    PM: serde::Serialize + DeserializeOwned + Send + Sync + 'static,
    GM: serde::Serialize + DeserializeOwned + Send + Sync + 'static,
{
    fn spawn(
        &self,
